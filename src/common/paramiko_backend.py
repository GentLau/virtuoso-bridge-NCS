...
class ParamikoTunnel:
    ...
    Paramiko has no high-level local-forward server API.  The forwarding
    channel itself is the official
    ``Transport.open_channel("direct-tcpip", ...)`` API; this class only adds
    the local listener/pump lifecycle around it.
...
class ParamikoSessionBackend:
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            raise RuntimeError(
                "The paramiko SSH backend requires the `paramiko` package. "
                "Install it with `pip install paramiko PySocks`."
            ) from exc
        ...
        if self._proxy is not None:
            try:
                ...
                raise RuntimeError(
                    "The paramiko SOCKS5 proxy requires the `PySocks` package. "
                    "Install it with `pip install paramiko PySocks`."
                ) from exc
            self._socks = socks
    ...
    def _parse_socks5_proxy(value: str | None) -> _Socks5Proxy | None:
        ...
        try:
            ...
            raise ValueError("ssh.proxy contains an invalid URL") from exc
        if (
            ...
            raise ValueError(
                "ssh.proxy must use socks5://host:port without credentials"
            )
        if port is None:
            raise ValueError("ssh.proxy must include an explicit port")
        if not 1 <= port <= 65535:
            raise ValueError("ssh.proxy contains an invalid port")
    ...
    def _proxy_jump_for(
        ...
        if proxy_command is not None and self._proxy is None:
            raise ValueError(
                ...
                "use ProxyJump, route.jump, or ssh.proxy"
            )
