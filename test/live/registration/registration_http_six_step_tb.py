...
Run with::
    ...
    PYTHONPATH=src python test/live/registration/registration_http_six_step_tb.py \
        --work-dir test/artifacts/env/reg-six --out test/artifacts/evidence/reg-six/evidence.json
"""
...
def _load_admin_token() -> str:
    """Admin credential for the control plane, never hardcoded in the repo.

    ``register.server`` stores only the SHA-256 of the built-in admin token and
    keeps the plaintext offline, so test code must not reintroduce it either.
    Resolution order: ``VB_ADMIN_TOKEN`` env var, then the gitignored file
    ``test/artifacts/admin-token.txt``.
    """
    ...
    raise SystemExit(
        "admin token required: export VB_ADMIN_TOKEN or create the gitignored "
        f"file {path}"
    )
...
def main() -> int:
    ...
    if args.local_mode:
        ...
        daemon = None
        if not args.root:
            # Deployed files are throw-away evidence.  Keep them inside the
            # TB work dir (never in the shared system temp area) and remove
            # them when the run finishes.
            local_root_tmp = work_dir / f"local-root-{uuid.uuid4().hex[:8]}"
            local_root_tmp.mkdir(parents=True, exist_ok=False)
