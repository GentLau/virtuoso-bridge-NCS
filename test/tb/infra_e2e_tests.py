...
def _case_file_roundtrip(transport) -> None:
    ...
    remote = "/tmp/vb_e2e_roundtrip.txt"
    ...
    _op(transport, "basic.command.run", cmd=f"rm -f {remote}")
...
def _case_demo(transport) -> None:
    ...
    pipeline = _op(
        ...
        remote_input="/tmp/vb_pipeline_in.txt",
        skill_code='printf("skill-ok")',
        command="cat /tmp/vb_pipeline_in.txt > /tmp/vb_pipeline_out.txt",
        remote_output="/tmp/vb_pipeline_out.txt",
