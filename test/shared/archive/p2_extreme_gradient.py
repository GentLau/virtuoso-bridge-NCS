...
def main() -> int:
    ...
    time.sleep(1)
    subprocess.Popen(["ssh", VPS, "mkdir -p /root/.virtuoso-bridge/p2/tmp && setsid nohup python3 /root/fake_daemon_host.py "
                      "--base-port 6900 --count 40 --token-prefix p2a > /root/.virtuoso-bridge/p2/tmp/p2.log 2>&1 < /dev/null &"],
                     stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, **no_window())
    subprocess.Popen(["ssh", "wsl-gent", "cd /home/Gent/vb-ncs && mkdir -p /home/Gent/.virtuoso-bridge/p2/tmp && setsid nohup "
                      ".venv/bin/python test/shared/fixtures/fake_daemon_host.py --base-port 6601 --count 60 "
                      "--token-prefix p2b > /home/Gent/.virtuoso-bridge/p2/tmp/p2-wsl.log 2>&1 < /dev/null &"],
                     stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, **no_window())
    time.sleep(5)

    wd = override_work_dir_for_tests(Path(tempfile.mkdtemp(prefix="vb-p2-")))
    ...
    for n in range(args.users):
        ...
        e.roles.daemon.root = (
            "/root/.virtuoso-bridge/p2" if host == VPS
            else "/home/Gent/.virtuoso-bridge/p2"
        )
        ...
        tokens.append(token)
    roots = [
        ("/root/.virtuoso-bridge/p2" if tok.startswith("p2a")
         else "/home/Gent/.virtuoso-bridge/p2")
        for tok in tokens]
