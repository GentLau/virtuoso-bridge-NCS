...
def main() -> int:
    wd = override_work_dir_for_tests(Path(tempfile.mkdtemp(prefix="vb-f2-")))
    ...
    e.roles.daemon.root = "/root/.virtuoso-bridge/recon"
    e.roles.daemon.expected_user = "root"
    reg.register("recon01", e)
    middle = BusinessServer(wd)

    def start_daemon():
        remote_dir = "/root/.virtuoso-bridge/recon/support"
        source = Path(__file__).resolve().parents[1] / "fixtures" / "fake_daemon_host.py"
        subprocess.run(["ssh", VPS, f"mkdir -p {remote_dir} /root/.virtuoso-bridge/recon/tmp"],
                       stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, **no_window())
        subprocess.run(["scp", str(source), f"{VPS}:{remote_dir}/"],
                       stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, **no_window())
        subprocess.Popen(["ssh", VPS, f"setsid nohup python3 {remote_dir}/fake_daemon_host.py "
                          "--base-port 6888 --count 1 --token-prefix recon "
                          "> /root/.virtuoso-bridge/recon/tmp/f2.log 2>&1 < /dev/null &"],
                         stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, **no_window())
        time.sleep(3)
