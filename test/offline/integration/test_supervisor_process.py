...
#: stores only its SHA-256; see ``src/register/server.py``).  Export it for the
#: run that needs the admin endpoints, e.g. from the gitignored file
#: ``test/artifacts/admin-token.txt``; otherwise the admin tests skip.
ADMIN = os.environ.get("VB_ADMIN_TOKEN", "")
_ADMIN_REASON = (
    "set VB_ADMIN_TOKEN (see test/artifacts/admin-token.txt) to exercise "
    "the admin endpoints"
)
