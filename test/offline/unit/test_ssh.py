...
class TestRunnerBackendDispatch(unittest.TestCase):
    ...
    def test_openssh_upload_text_uses_ssh_stdin(self):
        ...
        self.assertEqual(result.returncode, 0)
        self.assertTrue(run.called)
        # 全局 subprocess.run 被 patch 时，并发线程的调用也会进 call_args_list；
        # 只认"带 input 的那次上传调用"，不要用 call_args（最后一次调用）。
        payloads = [call.kwargs.get("input") for call in run.call_args_list
                    if call.kwargs.get("input")]
        self.assertTrue(any(b"hello" in payload for payload in payloads),
                        f"no upload payload found among {len(run.call_args_list)} subprocess call(s)")
...
class TestPortForwardLifecycle(unittest.TestCase):
    ...
    def test_tunnel_stderr_path_is_removed_on_stop(self):
        r = SSHRunner("server", user="u", backend="openssh")
        fd, name = tempfile.mkstemp(prefix="vb-tunnel-stderr-", suffix=".log")
        os.close(fd)
        path = Path(name)
        r._tunnel_stderr_path = path
        r.stop_port_forward()
        self.assertFalse(path.exists())

    def test_command_log_uses_process_local_rotating_file(self):
        import logging

        from common.ssh import _setup_command_log

        base = self.wd / "commands.log"
        process_log = self.wd / f"commands.{os.getpid()}.log"
        ...
        root.handlers = []
        try:
            _setup_command_log(base)
            logging.getLogger("probe").info("trigger %s", "y" * 100)
        finally:
        ...
        self.assertTrue(process_log.with_name(process_log.name + ".1").exists())
