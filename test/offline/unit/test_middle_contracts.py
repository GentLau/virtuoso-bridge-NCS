...
class TestFrozenInterfaceSignatures(unittest.TestCase):
    ...
    def test_query_token_is_keyword_only(self):
        ...
        for func in (BusinessServer.query, Middle.query):
            ...
            self.assertIn("role", params, func)
            self.assertIn("name", params, func)
            self.assertEqual(
                params["token"].kind,
                inspect.Parameter.KEYWORD_ONLY,
                f"{func} must take token as keyword-only",
            )
            self.assertEqual(params["role"].kind, inspect.Parameter.KEYWORD_ONLY)
            self.assertEqual(params["name"].kind, inspect.Parameter.KEYWORD_ONLY)
            self.assertIsNone(params["role"].default)
...
class TestQueryContract(unittest.TestCase):
    ...
    def test_query_filters_role_and_name(self):
        by_role = self.server.query(token="tok-q", role="command")
        self.assertEqual(list(by_role.roles), ["command"])
        self.assertEqual(
            by_role.roles["command"].calibre["version"],
            "2024.4",
        )

        by_group = self.server.query(
            token="tok-q", role="command", name="calibre",
        )
        self.assertEqual(list(by_group.roles), ["command"])
        self.assertEqual(
            by_group.roles["command"].calibre["bin"],
            "/opt/eda/mentor/calibre/bin/calibre",
        )

        by_root = self.server.query(
            token="tok-q", role="command", name="root",
        )
        self.assertEqual(
            by_root.roles["command"].model_dump(),
            {"root": "/home/alice/.virtuoso-bridge/alice/command"},
        )

        missing = self.server.query(
            token="tok-q", role="command", name="not_configured",
        )
        self.assertEqual(missing.roles["command"].model_dump(), {})

    def test_query_rejects_invalid_filters(self):
        for kwargs in (
            {"name": "calibre"},
            {"role": "unknown"},
            {"role": "command", "name": "host"},
            {"role": "command", "name": "daemon_port"},
            {"role": "command", "name": "Bad"},
        ):
            with self.subTest(kwargs=kwargs):
                with self.assertRaises(ValueError):
                    self.server.query(token="tok-q", **kwargs)
