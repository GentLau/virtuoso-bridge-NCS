...
#:
#: The production admin plaintext must never be committed (the server stores
#: only its SHA-256 and keeps the original offline — see
#: ``src/register/server.py``).  These tests swap the module-level hash for a
#: throwaway one so the admin paths stay covered without the real credential.
...
class TestManagementReload(unittest.TestCase):
    ...
    def test_update_triggers_reload(self):
        status, raw = self.srv.request(
            "POST",
            "/api/user/reload-user/update",
            {"roles": {"command": {"probe_group": {"value": 1}}}},
            _admin_auth(),
        )
        self.assertEqual(status, 200, raw)
        self.assertEqual(self.manager.calls, ["reload"])
        self.assertEqual(
            self.registry.get("reload-user").roles.command.probe_group,
            {"value": 1},
        )

    def test_remove_triggers_reload(self):
        status, raw = self.srv.request(
            "DELETE", "/api/user/reload-user", headers=_admin_auth(),
        )
        self.assertEqual(status, 200, raw)
        self.assertEqual(self.manager.calls, ["reload"])
        self.assertIsNone(self.registry.get("reload-user"))

    def test_update_reports_reload_failure(self):
        self.manager.reload_error = RuntimeError("business reload boom")
        status, raw = self.srv.request(
            "POST",
            "/api/user/reload-user/update",
            {"roles": {"command": {"probe_group": {"value": 2}}}},
            _admin_auth(),
        )
        self.assertEqual(status, 500, raw)
