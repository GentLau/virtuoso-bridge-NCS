...
class TestStrictJson(unittest.TestCase):
    ...
    def test_dumps_strict_escapes_lone_surrogates(self):
        text = dumps_strict({"value": "\ud800"})
        self.assertEqual(loads_strict(text)["value"], "\ud800")
        text.encode("utf-8")  # must never raise

    def test_dumps_strict_rejects_non_finite_numbers(self):
        for value in (float("nan"), float("inf"), float("-inf")):
            with self.subTest(value=value):
                with self.assertRaises(ValueError):
                    dumps_strict({"value": value})

    def test_dumps_strict_standard_round_trip(self):
        payload = {"a": [1, 2.5, True, False, None], "b": "\u4e2d\u6587"}
        self.assertEqual(loads_strict(dumps_strict(payload)), payload)
