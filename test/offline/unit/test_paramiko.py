...
class TestSshConfigLookup(unittest.TestCase):
    def setUp(self):
        self.cfg = Path(tempfile.mkdtemp(prefix="vb-")) / "config"
        self.cfg.write_text(
            "Host myserver\n"
            "  HostName real.example.com\n"
            "  User bob\n"
    ...
    def test_missing_config_file_raises(self):
        with self.assertRaises(FileNotFoundError):
            ParamikoSessionBackend(
                ...
                ssh_key_path=None, ssh_config_path=Path(tempfile.mkdtemp(prefix="vb-")) / "nope",
...
class TestBackendExecutionPaths(unittest.TestCase):
    ...
    def test_download_file_success(self):
        ...
        target = Path(tempfile.mkdtemp(prefix="vb-")) / "out.bin"
    ...
    def test_download_tar_failure_discards_stage(self):
        ...
        target = Path(tempfile.mkdtemp(prefix="vb-")) / "out"
    ...
    def test_upload_tar_failure_maps_to_transport(self):
        ...
        local = Path(tempfile.mkdtemp(prefix="vb-")) / "f.txt"
