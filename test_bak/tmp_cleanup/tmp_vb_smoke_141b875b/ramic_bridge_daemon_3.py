...
def _emit_stat(force: bool = False) -> None:
...
def _safe_sendall(conn: socket.socket, data: bytes) -> None:
    try:
        conn.sendall(data)
    except OSError:
        pass
...
def _parse_meta(payload: bytes) -> tuple[str | None, int]:
    """Second frame payload: ``<path> US <decimal-offset>``."""
    if US not in payload:
        return None, 0
    path_b, off_b = payload.split(US, 1)
    path = path_b.decode("utf-8", errors="replace") or None
    try:
...
def filter_delta(raw: str, level: str, max_bytes: int) -> tuple[str, bool]:
...
def _read_delta(path: str | None, end_offset: int) -> str:
