...
> 版本：Draft v2
> 日期：2026-09-10
> 状态：已冻结；最终语义索引到[配置一览](../总览/配置一览.md)
...
| daemon 端口 | 否 | 否 | 监听端口（缺省由注册表探测分配；最终语义见[配置一览](../总览/配置一览.md)） |
| 本地端口 | 否 | **不适用** | 仅远程模式：隧道本地端口（缺省自动选本地空闲端口；见[配置一览](../总览/配置一览.md)） |
| token | 否 | 否 | 缺省自动生成；手动指定用于团队约定/跨机迁移 |
| 目标文件安放地址（scratch root） | 否（不推荐） | 否 | 部署根；默认值见[配置一览](../总览/配置一览.md) `deploy.scratch_root` |
| 跳板主机/用户 | 否 | **不需要** | 需要 `-J` 时提供 |

> **没有 profile 参数，也不保留 profile 兼容**：旧架构的 profile（`VB_*_<profile>` 后缀）使命是"给配置集合起名"，注册表 + token 已把它取代；人类可读标签由用户名（`alice`）承担。
