...
| 2 注册表 | `registry.json` 落地（user 为键，冷启动只读）；六步注册流程（申请 → 本地校验 → 探测 → 部署 → 连通性测试 → 写入注册表）+ 注册表 `expected` 固化；registry 可按 `user` 查询（不提供 `bind()`/session 语义） | ✅ |
...
| 5 清理 | 删除 guard 强制路径与 `VB_ALLOW_CROSS_USER_DAEMON`，token 始终强制校验 | ✅ |
...
4. **token 取代 profile**（已明确）：profile 的全部旧职能（环境变量后缀/状态文件分离/部署目录叶子/venv 绑定）已由注册表 + token + 用户名取代；**profile 不保留任何兼容入口**；
