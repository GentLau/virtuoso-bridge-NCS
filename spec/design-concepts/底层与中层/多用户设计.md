...
|---|---|---|---|
| 注册第五步 | daemon 可达、token 比对、命令冒烟、Skill 冒烟、全 endpoint host-key | daemon/token/冒烟/指纹 → ERROR；banner、daemon_user 漂移 → WARNING | 端到端 30s；建连失败最多 3 次 |
...
endpoint 集合 = `{command.host, daemon.host, file.host, spectre.host}` 去重；每个 endpoint 的指纹存 `expected.ssh_endpoints`，key = `host|user|jump_host|jump_user|proxy`。
...
| 流 | token 的作用 | 说明 |
|---|---|---|
| Skill | daemon 侧**比对 il 注入的 token** + 选隧道端口 | token 不匹配 → 直接 NAK，SKILL 层完全不知道这次请求 |
...
| `user update` | 白名单字段（ssh.*、runtime.*、cdslog.*、deploy.scratch_root（需人工重部署）、route.spectre.host/bin、expected.ssh_endpoints）；副本修改→整体校验→原子覆盖；**成功后关闭该 token 的运行时缓存连接，下次调用重建** |
