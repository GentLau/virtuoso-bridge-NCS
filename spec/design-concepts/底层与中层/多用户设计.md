...
> Supersedes：Draft v11（补齐六步预测目标/失败合同、注册 deadline、resolver 复用口径）  
> 关联：[三层整体架构设计](../总览/三层整体架构设计.md)、[多节点设计](多节点设计.md)、[Spec 索引](../../README.md)

## 1. 目标

把一个**单连接点对点桥接**演进为**多用户接入架构**：

- 每个用户拥有独立的 resident daemon、独立监听端口；连接复用与预算的唯一口径见[多节点设计 §6](多节点设计.md)与[并发处理设计](并发处理设计.md)；
...
> **N 个用户 = N 个隔离域**。例如 6 个用户，上层看到的是 6 个业务服务器；每个隔离域包含五个 role、一个活动 daemon（= 一个 CIW）和一份 per-token 线程/channel 预算，中层按 token 参数路由到对应隔离域。跨用户并行互不阻塞；同账号场景下是桥层通道隔离（命令流不串、返回不混、文件落在各自 role 的根下）；**token 是核心区分**，OS 级权限分离由 IT 提供 Unix 账号保证，不在桥的职责内。接口清单与 role 映射见[三层整体架构设计 §4.1](../总览/三层整体架构设计.md)与[多节点设计 §3](多节点设计.md)，本文不重复。
...
本设计把上述机制**统一收拢**为六步注册流程（详见 3.2）：**①申请 → ②本地校验 → ③探测 → ④部署 → ⑤连通性测试 → ⑥写入注册表**。注册完成后按注册表路由；SSH 连接在 token 内按 endpoint 去重复用（唯一口径见[多节点设计 §6](多节点设计.md)与[并发处理设计](并发处理设计.md)）。
...
| 步 | 预测目标（可观察） | 不达成时 | 重试 |
|---|---|---|---|
| 1 申请 | 必填参数齐全（含强制字段 `mode.default`、用户名、各 role 主机/端口） | 参数错误，无副作用 | 修改后重新申请 |
| 2 本地校验 | registry 内无同名 user；`daemon_port` 在**同一 daemon 目标主机**上未被占用；`local_port` 在本机未被占用 | 返回冲突原因 | 改参数后重试 |
| 3 探测 | 五个 role 各自完成命令可用性探测；`expected_*` 固化；各 `role.*.root` 回写为绝对路径 | ERROR（spectre 专项仅 WARNING） | 修复环境后重试 |
...
**每步 deadline**：第二、三、五步的每个网络相位共享该步 30s 预算，`runtime.connect_timeout`（默认 15s）是其中的子预算、不额外加总；第四步之后**用户 load 的等待不计时**——第五步从用户确认、daemon 可观察时才开始计时。本表是注册侧 deadline 的唯一 owner；接口侧 deadline 见[三层整体架构设计 §5.8](../总览/三层整体架构设计.md)。
...
- 三层职责、五接口契约（Skill / 命令 / 文件 / GUI 命令 / Spectre 命令，见[三层整体架构设计 §4.1](../总览/三层整体架构设计.md)）、上层无感原则**全部不变**；具体字段与 framing 以该节为准；
