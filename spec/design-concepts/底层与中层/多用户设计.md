...
| 2 | daemon 主机名 | daemon role `hostname -f` → `expected.daemon_endpoint_hostname` |
| 3 | daemon 账号 | daemon role `whoami` → `expected.daemon_user` |
| 4 | daemon 端口 | daemon role 端口是否空闲（bind/占用检查，部署前可查）；是否真有 daemon 监听留到第五步 |
| 5 | python | daemon role 探测，决定 daemon 文件选型（_3 / _27） |
| 6 | 部署根 | daemon role `mkdir -p` + `test -w` |
| 7 | 命令冒烟 | command role（`echo vb-ok` 等部署前可跑项） |
| 8 | 文件根 | file role 根可见性（`file.host == daemon.host` 时随部署根；不同时校验绝对路径能力） |
| 9 | spectre | spectre role：bin 显式校验/自动探测，失败仅 WARNING |

任一失败 → **回退第一步**，返回结构化原因（如 `ssh handshake failed`、`daemon port 65081 already in use`、`deploy.scratch_root not visible on daemon host`）。
...
**五个 role 解析成 endpoint 后去重**，指纹存 `expected.ssh_endpoints`（key 算法见[配置一览 §6.5](../总览/配置一览.md)）。gui/daemon/command/file 的指纹是必检（ERROR）；spectre 本版不消费，其指纹仅记录、缺失/不匹配只 WARNING。
...
| 流 | token 的作用 | 说明 |
|---|---|---|
| Skill | daemon 侧**比对 il 注入的 token** + 选 daemon role 隧道端口 | token 不匹配 → 直接 NAK，SKILL 层完全不知道这次请求 |
| RunCommand | 仅**寻址**（选 command role），无安全校验 | 信任 SSH host key + 账号权限边界（见 5.5） |
| File | 仅**寻址**（选 file role + 默认根目录），无安全校验；默认目录只是路径约定，**同账号不构成跨用户安全隔离**（OS 权限边界） |
