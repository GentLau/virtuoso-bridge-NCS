...
接口与 wire framing 一律以[四层整体架构与接口 §4](../总览/1-四层整体架构与接口.md)为准，日志字段见[日志返回设计标准](日志返回设计标准.md)，本文件不重复。本设计只补充 token 边界：token 由中层放入发给 daemon 的请求，不进入 `ramic_bridge.il`，也不用于 `RunCommand`/`File` 的 SSH 认证。
