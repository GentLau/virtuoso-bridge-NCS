...
| 2 | 主机对不对 | SSH 握手取 host key 指纹并**记录到 `expected.ssh_endpoints`**（供第五步比对防机器漂移）+ `hostname -f` 得到主机名并**记录到 `expected.daemon_endpoint_hostname`** |
| 3 | 名字对不对 | `whoami` 得到登录账号；`id <daemon_user>` 校验 daemon 用户存在性；结果记录为 `expected.daemon_user`（供第五步比对） |
| 4 | 路径可不可写 | `mkdir -p` + `test -w` 部署根（daemon role 主机；deploy 与 daemon 同机，gui role 仅记录） |
