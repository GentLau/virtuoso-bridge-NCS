...
用户发起注册请求，提交**必选参数**（`mode.default`、用户名、各 role 主机/端口等）；`mode.default ∈ {local,remote}` 是强制字段，不自动推断；`role.*.mode` 可逐 role 覆盖。本步只是"提出申请"，不产生任何副作用。token 在此时生成（自动或手动指定），随申请在流程内部传递，第六步才落盘。
...
**前五步全部成功才落盘。** 写入完整注册条目（token、`mode.default`/`role.*.mode`、route、expected 探测值，见 3.4）。从此该 user 进入"已注册"状态，使用中按注册表路由（见 3.3）。
...
- **字段组成**：token、`mode.default`/`role.*.mode`、五个 role（gui/daemon/command/file/spectre，连接字段缺省回退全局 `ssh.default.*`）、expected（探测得到并记录的值）——**字段与示例见 [配置一览 §2](../总览/配置一览.md)**，此处不重复；
...
- 新增 `register.py`：**第三步探测**（对应功能到对应 role 探测：五 role SSH 握手取指纹与命令可用性；remote python / 端口空闲 / `role.daemon.root` 可写去 daemon role；各 role 文件根可写性去各自 role；spectre bin 去 spectre role）——只验部署前可验证项，任一必检失败返回原因、不落盘；**当前版本 daemon 是 CIW 的本地 `ipcBeginProcess` 子进程**；未来版本可能支持在其他节点启动，启动方式与运行账号由站点拓扑决定，bridge 不参与、不校验（忠实投送）；
...
| `user update` | 白名单字段（ssh.*、root.default、五个 role.*（含 root；改 daemon 端口/`role.daemon.root` 需人工重部署）、runtime.*、cdslog.*、expected.ssh_endpoints）；副本修改→整体校验→原子覆盖；**成功后关闭该 token 的运行时缓存连接，下次调用重建**。写方进程立即生效；**其他已运行进程按本版 load-once 语义不自动感知，需重启或显式 reload 后才生效**（见 §3.4 冷启动） |
