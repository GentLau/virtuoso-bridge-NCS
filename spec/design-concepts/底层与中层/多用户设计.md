...
| 8 | 文件根 | file role 根可见性（file 与 daemon 同一 endpoint 时随部署根；不同时校验绝对路径能力） |
| 9 | spectre bin | spectre role：bin 显式校验/自动探测，失败仅 WARNING |

任一**必检**项失败 → **回退第一步**，返回结构化原因（如 `ssh handshake failed`、`daemon port 65081 already in use`、`deploy.scratch_root not visible on daemon host`）。spectre role 的探测失败只产生 WARNING，不阻断注册。逐 role 的职责与失败级别唯一 owner：[多节点设计](多节点设计.md)。
...
- 新增 `register.py`：**第三步探测**（对应功能到对应 role 探测：五 role SSH 握手取指纹；remote python / 端口空闲 / scratch_root 可写去 daemon role；命令冒烟去 command role；文件根可见性去 file role；spectre bin 去 spectre role）——只验部署前可验证项，任一必检失败返回原因、不落盘；daemon 由 CIW 的 `ipcBeginProcess` 以 daemon role 的账号启动，不提供 sudo/setuid/run-as；
...
| `user update` | 白名单字段（ssh.*、五个 role.*（改 daemon 端口/scratch_root 需人工重部署）、runtime.*、cdslog.*、expected.ssh_endpoints）；副本修改→整体校验→原子覆盖；**成功后关闭该 token 的运行时缓存连接，下次调用重建** |
