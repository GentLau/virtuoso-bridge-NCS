...
| 4 | 路径可不可写 | `mkdir -p` + `test -w` 部署根（daemon 主机即 deploy 主机，与 GUI 同机；本版无独立 GUI role） |
...
### 3.3 使用中的路由与并发（每 token×endpoint 一条 SSH）

**注册完成后，使用期的核心约定：每 token × 每个不同 SSH endpoint 一条连接，按注册表路由。** 并发细节见 [并发处理设计](并发处理设计.md)，此处只列与路由相关的决策点。
...
- **每 token × 每个不同 SSH endpoint 一条连接**（+ 每 token 独立线程池 + 每 endpoint channel 预算）——严禁跨 user/跨 endpoint 复用；两个 user 连同一台机器也是两条 SSH（详见并发处理设计 §11）；
- 路由表**只在中层内部可见**，上层拿不到；找不到 user、目标不可达 → 结构化错误返回，不猜测、不回退。

**连通性测试只有三个时机，业务调用本身零测试**：

| 时刻 | 校验范围 |
|---|---|
| 注册第五步 | 完整：daemon 可达 + token 比对 + 全 endpoint host-key + banner 主机名 + 命令冒烟 `echo vb-ok` + Skill 冒烟 `1+1==2` |
| runtime 冷启动 | 建连接 + 隧道就绪 + 首次 Skill 冒烟；不重跑完整探测 |
| 断线重连 | 同冷启动，并比对 banner 主机名 |
| 之后每次 `execute_skill`/`run_command` | 零测试、零探测；`execute_skill` 仅由 daemon 比对 token，`run_command` 直接走该 token 的 SSH endpoint |
...
- **字段组成**：token、mode、route（skill/command/file 三路）、expected（探测得到并记录的值）——**字段与示例见 [配置一览 §2](../总览/配置一览.md)**，此处不重复；
...
| File | 仅**寻址**（选 file host + 默认根目录），无安全校验；默认目录只是路径约定，**同账号不构成跨用户安全隔离**（OS 权限边界） |
...
| stop/remove 语义 | **已定**：`stop` 保留注册；`user remove` 才销毁 token |
| 一个 token 多会话 | **已定**：一份部署一个 token，可被同一用户的多个 CIW load；如需按会话隔离，注册第二个 user |
...
| 操作 | 本版语义 |
|---|---|
| `user remove` | 原子移除 registry 条目；若同进程存在该 token 的运行时客户端，关闭其缓存连接与隧道。**不**停 daemon、**不**删远端部署目录——daemon 随 CIW 生命周期，远端文件是用户资产；如需清理由用户/运维另行操作 |
| `user update` | 只允许修改白名单字段（ssh.*、runtime.*、cdslog.*、`deploy.scratch_root`（需人工重部署）、route.spectre.host/bin、`expected.ssh_endpoints` 指纹）；用副本修改 → 整体校验 → 原子覆盖，失败不污染原条目 |
| 用户改名 / route 大改 | 不支持原地改名；一律 `remove` 后重新注册 |
| 机器重装（host key 变化） | 用户带外确认新指纹后，显式 `update` `expected.ssh_endpoints` 对应条目；未确认前连接按指纹不匹配拒绝 |
| 删除失败的一致性 | 以 registry 写入结果为准：删除成功即注册侧完成，远端状态与 registry 解耦、不做远端回滚 |
| WARNING 消费 | `VirtuosoResult.warnings` / 注册 report.warnings 结构化暴露；上层或 CI 可自行决定是否升级为失败 |
