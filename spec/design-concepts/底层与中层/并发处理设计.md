...
> 版本：Draft v13
> 日期：2026-09-14
> 状态：Normative（并发、预算与连接复用唯一口径）
> Supersedes：Draft v12（单连接上限配置化为 role.max_sessions）
...
| # | 事实 | 后果 |
|---|---|---|
| 1 | 每个用户的 daemon 是**独立 OS 进程**（当前版本由 CIW 的本地 `ipcBeginProcess` 拉起；未来可能在其他节点启动） | 真并行——一个忙不阻塞另一个 |
...
**真相：等待不占 CPU，但占线程、socket、内存和 channel 配额。** 进程在 `recv()` 阻塞时被内核挂起，CPU 占用 = 0；但每个在途请求仍占一个**线程**（per-token 线程预算，所有 role 共享），remote role 还占一个 socket 与（一次性通道的）channel 记账名额。哪些动作占 channel、哪些不占，唯一矩阵见 §11.2。所以**一个进程可以同时"等"很多请求**——它们全部挂在各自 socket 的内核接收队列上睡觉，谁的数据先到，内核先叫醒谁。
...
用户加载 setup 启动 daemon（常驻；当前版本为 CIW 本地 `ipcBeginProcess` 子进程，生命周期随 CIW；未来可能在其他节点启动）
...
| 情形 | 结局 | 语义 |
|---|---|---|
| 并发数 ≤ backlog | 排队等待 | TCP 语义（内核排队） |
| 并发数 > backlog | 连接被拒（错误文本与形态由 OS/实现决定） | 超限语义，由中层错误合同处理 |

> **本版口径**：daemon 使用 `listen(128)` 作为实现参数；backlog 是**容量提示**，不是跨平台确定性阈值（不同 OS/内核下超额表现可能不同）。上层语义固定为：**连接失败即结构化错误，可重试**；中层每 token 线程池（默认 32）先于 daemon 限流。
...
布局：alice、bob → 1 号机器；carol → 2 号机器；同一 token 内按 endpoint 去重——每台目标机器一条**业务连接**（命令/文件 channel 复用）；remote daemon 另有一条专用 `-L` 隧道（独立连接）。**本例只列 remote role 的 SSH 账本**；`mode=local` 的 role 不产生 SSH 连接，也不占 channel（见 §11.2 矩阵）。

```text
1 号机器：
  sshd 前台        1 个（监听 22，只"接客"）
  sshd 子进程      2 个（按 SSH 连接数 fork：alice、bob 各一）
                  每个子进程内部 N 个逻辑 channel（命令/文件等业务 channel；`-L` 隧道是独立连接，不在此子进程内）
  daemon           2 个（alice、bob 各自的独立 OS 进程，与 sshd 无父子关系）
2 号机器：
...
| 纪律 | 内容 | 违反后果 |
|---|---|---|
| **注册表只读共享** | 冷启动读一次 → 载入连接对象内存；之后不再访问 | 反复读写则需加锁，不加锁则数据竞争 |
...
复用机制：**ControlMaster** + **persistent shell**，三级降级（persistent shell → ControlMaster → 裸 ssh）。**隔离口径（已定）**：线程池与 channel 预算**每 token 一份**、token 内独立管理，与 endpoint 无关（记账唯一矩阵见 §11.2）；SSH 连接同一 token 内按 endpoint 去重复用——ControlPath 含 token 的不可逆 hash（多 endpoint 时再按 endpoint 分段），跨 token 不共享；性能由 token 内连接复用保证，关闭只影响本 token。
...
布局：Alice、Bob → 主机 192.168.1.100（daemon 端口 65080、65081）；Claire → 192.168.1.200（daemon 端口 65080，不同主机不冲突）。三人已各自建立 SSH 隧道。**注意：下列 6 个请求全部由 Alice 一人发出**——本案例考的不是"多用户"，而是"单用户多请求 × 三条通道（子集示例）"的交叉并发。

> **这是子集示例**：只演示 Skill / 命令 / 文件三条流，用来讲清"锁在哪里、排队就在哪里"。它不是接口全集，也不改变任何预算规则——GUI 命令、Spectre 命令与 `parallel=True` 的记账见 §11.2 矩阵。
...
**这几条通道之间：完全互不阻塞**——skill A 在 daemon 里跑时，cmd A 的 shell 照常跑、文件照常传。它们是三个独立执行位置（daemon 进程 / 远端 shell / SCP 进程），谁也不占谁的线。
...
- Alice 的 6 个请求中，命令×2 + 文件×2 在 Alice 的业务连接内多路复用 channel，占用都在 Alice 那一份 channel 预算（默认 10）之内；Skill 由 daemon role 的专用隧道连接承载，不在业务连接的 channel 账本内。
...
**设计倾向**：**默认串行，显式并发**——保持 persistent shell 的串行语义作默认（保证命令依赖顺序），上层想并行跑独立命令时，显式并发 = `run_command(..., parallel=True)`，中层分配独立 exec channel。理由：命令间的依赖（编译→运行、生成→读取）是常态，串行默认防止静默乱序；并行是少数需求，应显式表达。并行只是 `run_command` 的 `parallel=True` 参数，不新增接口；接口总数固定为 5（见[三层整体架构设计 §4.1](../总览/三层整体架构设计.md)）。
...
**子集示例的结论（只覆盖 Skill/命令/文件三条流，不构成接口全集）**：
...
| 资源 | 每 token 拥有 | 规则 |
|---|---|---|
| SSH 连接 | 同一 token 内按 endpoint 去重，每个 endpoint 1 条 | 不同 token 即使 endpoint 相同也不共用连接 |
| 线程池 | 每 token 1 份 | **严禁跨 token 复用**；默认 32 线程，可调 |
| channel 预算 | 每 token 1 份 | token 内所有 endpoint 共用这一份；默认 10，可调 |

**线程池机制（已定）**：采用线程池（非每请求新建线程）。线程干完回池复用，不销毁；池收缩（shutdown）才真正销毁。**超过线程上限（默认 32）直接返回错误值**，不排队等线程（与 channel 记账的唯一矩阵见 §11.2）。一次调用的生命周期：收到调用 → 按 token 匹配路由 → 在该 token 的线程池取一个线程 → 线程负责投送并在 `recv()`/`waitpid()` 中挂起等待 → 结果返回后线程回池。命令未完成期间该线程一直被占用（不占 CPU，但占线程/socket/内存配额）。

**连接建立重试（已定，继承旧实现）**：建连失败不排队、不加预算参数——只在确认“未产生副作用”的握手阶段**最多 3 次总尝试（含初次）**（瞬时 banner/连接重置 + ControlMaster 故障降级重试），重试仍失败即返回 `VB-TRANSPORT:` 错误。上层无感知：它只看到最终成功/失败，不关心重试了几次。
...
| 命令执行 | **串行**（默认） | persistent shell 单会话 | 队列等待 |
| | 并行（**`parallel=True`**） | 独立 exec channel，一次性 | 见 channel 记账 |
| 文件执行 | **并行**（**天然，无需声明**） | 独立 SCP/SFTP 进程，每路一个 channel | 见 channel 记账 |
| GUI 命令执行 | **并行（一次性）** | 每次调用一条一次性 exec channel，无常驻 shell | 见 channel 记账 |
| Spectre 命令执行 | **并行（一次性）** | 每次调用一条一次性 exec channel，无常驻 shell | 见 channel 记账 |

**命令通道的"默认串行、显式并行"**（§10.3 的决策落地）：上层默认走 persistent shell（串行，保证依赖顺序）；需要并行跑独立命令时显式声明，中层分配独立 exec channel。上表只描述并发形态；**每个动作占不占 channel 预算，唯一答案是 §11.2 矩阵**。

**文件通道的"无条件并行"**：文件传输不存在串行模式可切换——每路传输本来就是独立进程 + 独立 channel，**没有任何参数要传**。并行是它的唯一形态。

### 11.2.0 local role 等价语义（已定）

`role.mode=local` 与 `role.mode=remote` 对外并发语义一致：

| 通道 | `role.mode=local` 规则 |
|---|---|
| Skill | 每 token 独立 `SkillClient` 直连 daemon；daemon 单线程串行，backlog 128 排队 |
| 命令 `parallel=False` | **每 token 一个常驻命令解释器（Windows cmd / POSIX sh），复用会话**：命令写进同一 stdin、按序执行，**cwd 与 shell 变量跨命令保留**（与 remote persistent shell 等价）；解释器由该 token 的锁保护 |
| 命令 `parallel=True` | 独立本地子进程，无锁、不共享 cwd/env |
| 文件 | 独立本地 copy/copytree，天然并行 |

local role **不占 channel 预算**（没有 SSH channel），但**照常占线程预算**：`thread pool exceeded` 语义与 remote 完全一致。

**共享规则（无特例）**：线程预算与 channel 预算都只有**每 token 一份**。同一个 token 里混用 local 与 remote、或 remote 分布在多个 endpoint，都从这两份预算里扣；不做 role 级、endpoint 级或 local/remote 级的再分配。
...
| persistent shell（串行命令） | 长驻 | 保活 + 失效重建（§3；**重发规则见[三层整体架构设计 §5.8](../总览/三层整体架构设计.md)**） |
...
**channel 记账矩阵（唯一口径：每 token 一份，token 内所有 role/endpoint 共享）**：

| 动作 | 线程预算 | channel 预算 | 释放时机 |
|---|---|---|---|
| 任意接口的在途调用（五个接口都算） | 占 1 | 见本表对应行 | 调用返回 |
| Skill 执行（隧道转发） | 占 1 | **不占**：forwarding channel 由 `ssh -L` 隧道连接自身承载，并发上限由线程预算与远端 sshd 能力约束 | 调用返回 |
| 命令执行（默认串行，常驻 shell） | 占 1 | **不占**：常驻 shell 是长驻会话，不参与一次性 channel 记账；同一 token 的命令在该 shell 内串行 | 调用返回（shell 常驻，不随调用销毁） |
| 命令执行（`parallel=True`） | 占 1 | **占 1** | 命令结束（exec channel 由远端自动关闭） |
| 文件执行（上传/下载，**含结束后的校验子命令**） | 占 1 | **占 1**（校验子命令在同一名额内完成，不另占） | 传输与校验都结束 |
| GUI 命令执行 | 占 1 | **占 1** | 命令结束 |
| Spectre 命令执行 | 占 1 | **占 1** | 命令结束 |
| `mode=local` 的 role（以上任一动作） | 占 1 | **不占**（该 role 不涉及 SSH） | 调用返回 |

- 两份预算独立计数：`runtime.thread_pool_size`（默认 32）与 `runtime.channel_budget`（默认 10），都可按用户调整；
- **所有 role 共用同一份预算**，token 内无论多少个 role / endpoint，都从这两份里扣，没有 role 级或 endpoint 级特例；
- 超限行为：线程超限 → `thread pool exceeded`；channel 超限 → `channel budget exceeded`；两者都是 `kind=rejected`（可重试），不排队等待。

GUI/Spectre 命令与 `parallel=True` 的命令并发**本质相同**：都是一次性 exec channel；区别只是目标 role 不同（gui / spectre），且不提供常驻 shell 形态。
...
| channel 预算 | 依赖远端 sshd 默认 MaxSessions | **每 token 一份显式记账** + 可调上限；一次性 channel 用完自动销毁；远端 MaxSessions 是环境能力，由 `role.<name>.max_sessions`（默认 10，每 role 一条远端连接）本地代理、可配置 |
...
> 并发的地基是**进程隔离**（N 个 daemon = N 个 OS 进程，远端真并行）；并发的方式是**睡下等叫**（所有参与者阻塞在系统调用上，内核按数据到达唤醒，谁先完成谁先回）；并发的代价是**零**（等待期间 GIL 释放，计算只有微秒）；要守的纪律是**无共享可变状态**（注册表只读、连接私有、禁全局变量）。**并发配置（已定）**：每 token 一份线程池（默认 32，超限报错）+ 一份 channel 预算（超限拒绝）；SSH 连接同一 token 内按 endpoint 去重复用；Skill 串行（语义）、命令默认串行、显式并行（run_command 的 `parallel=True`）、文件默认并行。服务形态补上连接池与同 token 限流，就是完整的非阻塞多用户架构。
