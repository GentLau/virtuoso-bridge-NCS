...
**连接建立预算（已定）**：每个 SSH endpoint 有独立的“连接建立”并发预算 `ssh.connection_budget`（默认 16）。首次建连/断线重连的手势在该 endpoint 内排队（等待时间继承端到端 deadline），不同 endpoint 互不排队；超过预算+deadline 则返回 `VB-TRANSPORT:` 错误。这是缓解 sshd `MaxStartups` 的建连背压，**不跨用户串行业务**。
...
| 命令 `parallel=False` | **每 token 一把串行锁**，保先后顺序（与 remote persistent shell 等价）；不共享 cwd/env |
| 命令 `parallel=True` | 独立本地子进程，无锁、无共享状态 |
