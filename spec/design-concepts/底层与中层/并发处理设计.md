...
| 连接种类 | 建立成本 | 是否需池化 |
|---|---|---|
| execute_skill 的 TCP（连本地隧道端口） | 微秒级 | 不需要（短连接即可，见 protocol-v1 的"一条连接=一请求一响应"设计） |
| SSH 会话（跑命令的 shell） | **秒级握手** | **必须池化** |

复用机制现有代码已具备：**ControlMaster**（同 host 所有 ssh 调用共享一条 TCP，"N 次冷握手 → 1 次"）+ **persistent shell**（复用远端常驻 shell），三级降级（persistent shell → ControlMaster → 裸 ssh）。server 化要补的是**池化调度层**：按 token 找已有连接实例、没有才建、建了存起来、断了重建（user 只是中层索引）。
...
| 资源 | 每 token 拥有 | 规则 |
|---|---|---|
| SSH 连接 | 1 条 | **严禁跨 token 复用**——两个 token 连同一台机器，也是两条 SSH |
...
| SSH 复用 | ControlMaster 同 host 全局共享一条 TCP | **每 token 独立 SSH**；token 内部才复用 channel |
