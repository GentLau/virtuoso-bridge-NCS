...
**真相：等待不占 CPU，但占线程、socket、内存和 channel 配额。** 进程在 `recv()` 阻塞时被内核挂起，CPU 占用 = 0；但每个在途请求仍占一个线程、一个 socket 和一个 channel 记账名额。所以**一个进程可以同时"等"很多请求**——它们全部挂在各自 socket 的内核接收队列上睡觉，谁的数据先到，内核先叫醒谁。
...
布局：alice、bob → 1 号机器；carol → 2 号机器；同一 token 内按 endpoint 去重——对每台目标机器一条 SSH，内含命令 channel + `-L` 隧道转发。
...
复用机制：**ControlMaster** + **persistent shell**，三级降级（persistent shell → ControlMaster → 裸 ssh）。**隔离口径（已定）**：线程池与 channel 预算**每 token 一份**、token 内独立管理，与 endpoint 无关；SSH 连接同一 token 内按 endpoint 去重复用——ControlPath 含 token 的不可逆 hash（多 endpoint 时再按 endpoint 分段），跨 token 不共享；性能由 token 内连接复用保证，关闭只影响本 token。
...
**线程池机制（已定）**：采用线程池（非每请求新建线程）。线程干完回池复用，不销毁；池收缩（shutdown）才真正销毁。**超过线程上限（默认 32）直接返回错误值**，不排队等线程。一次调用的生命周期：收到调用 → 按 token 匹配路由 → 在该 token 的线程池取一个线程 → 线程负责投送并在 `recv()`/`waitpid()` 中挂起等待 → 结果返回后线程回池。命令未完成期间该线程一直被占用（不占 CPU，但占线程/socket/内存配额）。
...
`mode=local` 的 role 不涉及 SSH，因此该 role 无 ControlMaster/channel 预算；线程池与 `thread pool exceeded` 语义完全一致（预算仍按 token 计）。
...
**channel 记账（每 token 一份，token 内独立管理）**：

```text
每 token 的 channel 占用（与 endpoint 无关，token 内所有 endpoint 合计）：
  命令串行 shell       ×1   （persistent shell，常驻）
  命令并发通道         ×N   （parallel=True 时，每个在途并发命令一个，用完自动销毁）
  文件传输             ×M   （每路在途上传/下载一个，用完自动销毁）
  GUI 一次性命令       ×G   （每个在途 GUI 命令一个，用完自动销毁）
  Spectre 一次性命令   ×S   （每个在途 Spectre 命令一个，用完自动销毁）
  ──────────────────────
  合计 ≤ 该 token 的 channel 预算（runtime.channel_budget，默认 10，可调）

不记账：Skill 的隧道 forwarding channel——它由隧道连接（ssh -L）自身承载，
并发上限由该 token 的线程池（默认 32）和远端 sshd 能力共同约束，不重复扣预算。
```

GUI/Spectre 命令与 `parallel=True` 的命令并发**本质相同**：都是一次性 exec channel；区别只是目标 role 不同（gui / spectre），且不提供常驻 shell 形态。

**超过 channel 预算 → 拒绝**（`channel budget exceeded`，可重试），不排队等待——因为 channel 超限意味着该 token 已到硬顶，排队只会无限堆积。
