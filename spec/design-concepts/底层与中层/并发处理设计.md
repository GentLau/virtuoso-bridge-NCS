...
| 每次 `run_command` 的 SSH channel + 远端临时 shell | 一命令一个 channel，shell 跑完即退 |
...
复用机制：**ControlMaster**（同 host/user/jump 可自动合并共享一条 TCP）+ **persistent shell**（复用远端常驻 shell），三级降级（persistent shell → ControlMaster → 裸 ssh）。**隔离口径（已定）**：业务层的线程池与 channel 预算必须 per-token 记账；底层 SSH 允许按 host/user/jump 自动合并（性能优先），关闭时按引用计数只拆无使用者的 master，不误关其他 token。
...
| 资源 | 每 token 拥有 | 规则 |
|---|---|---|
| SSH 连接 | 业务层 per-token | 线程池/channel 预算 per-token 隔离；底层 SSH 允许按 host/user/jump 自动合并复用（如 ControlMaster） |
...
| SSH 复用 | ControlMaster 同 host 全局共享一条 TCP | 业务层 per-token 隔离；底层 SSH 可按 host/user/jump 自动合并，关闭按引用计数只拆自己的 |
