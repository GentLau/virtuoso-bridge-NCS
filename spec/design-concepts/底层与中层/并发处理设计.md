...
> Supersedes：Draft v10（重发规则指向架构 owner）
...
布局：alice、bob → 1 号机器；carol → 2 号机器；同一 token 内按 endpoint 去重——对每台目标机器一条 SSH，内含命令 channel + `-L` 隧道转发。**本例只列 remote role 的 SSH 账本**；`mode=local` 的 role 不产生 SSH 连接，也不占 channel（见 §11.2 矩阵）。
...
- Alice 的 6 个请求在 Alice 自己的 SSH 连接内多路复用 channel（命令×2 + 文件×2；隧道连接单独承载 Skill 转发），占用都在 Alice 那一份 channel 预算（默认 10）之内。
...
**连接建立重试（已定，继承旧实现）**：建连失败不排队、不加预算参数——只在确认“未产生副作用”的握手阶段**最多重试 3 次**（瞬时 banner/连接重置 + ControlMaster 故障降级重试），重试仍失败即返回 `VB-TRANSPORT:` 错误。上层无感知：它只看到最终成功/失败，不关心重试了几次。
