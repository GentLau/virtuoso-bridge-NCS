...
| 变量 | 作用 | 类型 | 默认值 |
|---|---|---|---|
| `expected.ssh_endpoints` | **所有 `mode=remote` 的 role** 解析后的 endpoint → host-key 指纹 map；key 见 §6.5，值 = `SHA256:<fingerprint>`；**mode=local 的 role 无本条**；spectre 失败不写入，其余 remote 业务 role 必检 | 校验 | 必填（注册探测；无 remote role 时为空） |
...
| `expected.ssh_endpoints`（显式指纹） | 校验 | 否 | 用户显式提供 per-endpoint 指纹则校验后记录；key/算法见 §6.5 |
...
2. **校验比对基准**：`expected.ssh_endpoints`（**所有 `mode=remote` 的 role** 解析后的 endpoint；local role 无 SSH、不产生指纹）、`expected.daemon_endpoint_hostname`、`expected.daemon_user` 由探测获得并记录，供第五步/运行期比对；不一致按 §4.1 的 WARNING（host-key 例外 ERROR）处理，不参与业务。
...
- `mode.default = "remote"`、`role.*.mode` 全为 `null` → 五个 role 全部 remote；`role.*` 连接字段全为 `null` → 全部回退 `ssh.default.*`，五个 role 解析成同一 endpoint `["server-a","ssh-user","","",""]`，因此 `ssh_endpoints` 只有一项（key 算法见 §6.5）。
