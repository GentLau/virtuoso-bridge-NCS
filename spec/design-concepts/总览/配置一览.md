...
> Supersedes：Draft v15（reservation 跨平台模型与唯一性作用域、root 定义索引化）；注册可提交参数目录见 §4，canonical 注册 API 见 §6。
...
| 变量 | 作用 | 类型 | 默认值 |
|---|---|---|---|
| `ssh.backend` | SSH 后端 openssh / paramiko。paramiko 实测更快（命令/文件约 3–4 倍，直连与 ProxyJump 均可用），但**作为默认时 Skill 隧道未打通**（两次真机复现，含换用全新本地端口），故本版默认仍为 openssh；Skill 隧道始终由外部 `ssh -N -L` 承载（与历史实现一致） | 配置 | 默认 `openssh` |
...
| `runtime.thread_pool_size` | **每 token 一份**线程池：同时在途请求上限（所有 role 共享）；超限返回 `thread pool exceeded` | 配置 | 默认 `32` |
| `runtime.channel_budget` | **每 token 一份** channel 预算，token 内所有 role/endpoint 共享；哪些动作占用见[并发处理设计 §11.2](../底层与中层/并发处理设计.md) | 配置 | 默认 `10` |
...
| `root.default` | 各 role 文件根的**全局默认基准**；role 未显式配 `root` 时其根 = `root.default/<role>`。`~` 与 `<userid>` 在**该 role 工作所在机器**上解释，与 mode 无关（唯一算法：[多节点设计 §5](../底层与中层/多节点设计.md)） | 配置 | 默认 `~/.virtuoso-bridge/<userid>` |
...
daemon role 根下的固定目录（写死，不配置；根本身已含用户段，算法见[多节点设计 §5](../底层与中层/多节点设计.md)）：
...
| CDS.log 读取 | daemon 本地读；offset/frame/限长的唯一口径见[日志返回设计标准](../底层与中层/日志返回设计标准.md) |
...
|---|---|---|---|
| `local` | **不适用**；显式提交即参数错误 | `role.<name>.root`（缺省 `root.default/<role>`），在**该 role 工作所在机器**上解释；算法见[多节点设计 §5](../底层与中层/多节点设计.md) | 在客户端本机执行等价检查；无 SSH、无 host-key |
...
1. **申请（第二步）**：写入自己的候选记录（同锁内先校验 registry 与已有记录），命中冲突即回退第一步；
2. **可见性**：其他注册流程在第二步同时检查 registry 条目与该文件中的记录，命中即报冲突，不进入第三步；
3. **TOCTOU 二次确认**：第三步在目标主机做 bind/占用探测；第四步部署前再次确认端口空闲，**冲突则自动重新分配并在同一锁内原子更新自己的记录**后重试一次，仍冲突即拒绝注册；
4. **释放**：第六步 commit 成功、或注册失败/取消时删除自己的记录（同一锁内原子替换）；
5. **崩溃与恢复**：记录带 `pid` 与时间戳。读取方发现记录 `pid` 已不存在、或 `updated_at` 超过 `stale_after`（默认 15 分钟）时视为陈旧并回收；**重启/换机器后按同一规则自动清理**，不产生永久 reservation（Windows 下 pid 可能被复用，因此以时间戳为兜底判据）；
...
1. **字符串规范化**（上表）决定"两个写法是不是同一个 endpoint"：`Server-A`、`server-a.`、`server-a` 规范化后相同 → **同一个 endpoint**（共用一条 SSH 连接与 ControlPath，`expected_fingerprint` 必须一致）；
2. **不做名称解析**：bridge 不查 DNS、不读 `~/.ssh/config` 做映射，也不把 alias 与真实 hostname 视为同一 endpoint——`prod-alias` 与 `prod.example.com` 是否同一台机器**不由 bridge 判断**，它们是两个 endpoint。
