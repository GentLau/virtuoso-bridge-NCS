...
| 变量 | 作用 | 类型 | 默认值 |
|---|---|---|---|
| `ssh.backend` | SSH 后端 openssh / paramiko。**默认 paramiko**：命令/文件/GUI/Spectre 在同一业务连接上多 channel 复用（不依赖 ControlMaster，Windows 亦有效）；**Skill 隧道由外部 OpenSSH `ssh -N -L` 承载**。连接数量与复用的唯一口径见[多节点设计 §6](../中层/多节点设计.md) | 配置 | 默认 `paramiko` |
...
`role.<name>.mode` 决定该 role 的投送方式：`local` = 客户端（中层）就在该 role 的目标主机上，直接本地执行、不经 SSH；`remote` = 经 SSH 投送到该 role 的 host/user。**不同 role 可以取不同 mode**（例如 daemon=local、command=remote）。role 的职责、探测矩阵与失败级别见[多节点设计](../中层/多节点设计.md)，本节只定义字段与默认值。
...
| `root.default` | 各 role 文件根的**申请期基准**：role 未显式配 `root` 时，申请期以 `root.default/<role>` 计算，**探测后把最终绝对路径写回各 `role.*.root`，运行期不再依赖本字段**；`~` 与 `<userid>` 在**该 role 工作所在机器**上解释，与 mode 无关（唯一算法：[多节点设计 §5](../中层/多节点设计.md)） | 配置 | 默认 `~/.virtuoso-bridge/<userid>`（申请期展示；持久化默认 `null`） |
...
daemon role 根下的固定目录（写死，不配置；根本身已含用户段，算法见[多节点设计 §5](../中层/多节点设计.md)）：
...
|---|---|---|---|
| `local` | **不适用**；显式提交即参数错误 | `role.<name>.root`（缺省 `root.default/<role>`），在**该 role 工作所在机器**上解释；算法见[多节点设计 §5](../中层/多节点设计.md) | 在客户端本机执行等价检查；无 SSH、无 host-key |
...
1. **字符串规范化**（上表）决定"两个写法是不是同一个 endpoint"：`Server-A`、`server-a.`、`server-a` 规范化后相同 → **同一个 endpoint**（`expected_fingerprint` 必须一致；连接数量与复用见[多节点设计 §6](../中层/多节点设计.md)）；
...
- 同 key = 同一 endpoint：共享该 token 的业务连接与 fingerprint 基准（拓扑 owner：[多节点设计 §6](../中层/多节点设计.md)）；不同 key 不共享；
