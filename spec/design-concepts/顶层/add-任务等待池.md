...
> Supersedes：Draft v2（任务池只服务不等待调用，去掉阻塞/非阻塞概念）
...
- 任务池**只服务“不等待”的调用**：业务包返回 `pending` 即登记一个任务，由挂起区监督；任务池没有“阻塞/非阻塞”概念，也没有 `wait` 参数；
- 阻塞等待是**业务包自己的同步行为**（例如仿真 operation 同步等到完成才返回，期间自己怎么查询与任务池无关），不经过任务池；
