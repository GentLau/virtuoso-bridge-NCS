...
> Supersedes：Draft v19（索引任务等待池端点）
...
- 业务端口只做 operation 调度，不开注册/管理/配置端点；`query` 是上层↔中层接口，不是 HTTP 端点；任务等待池端点（`GET/DELETE /api/job/<job_id>`、`wait` 参数）见[任务等待池](add-任务等待池.md)；
