...
> Supersedes：Draft v20（任务等待池无阻塞/非阻塞概念，去掉 wait 参数）
...
- 业务端口只做 operation 调度，不开注册/管理/配置端点；`query` 是上层↔中层接口，不是 HTTP 端点；任务等待池端点（`GET/DELETE /api/job`）见[任务等待池](add-任务等待池.md)；
