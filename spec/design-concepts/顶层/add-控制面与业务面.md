...
> Supersedes：Draft v19（新增 POST /api/bug 缺陷提交端点）
...
| POST | `/api/bug` | 提交 bug 报告（格式不做要求，原文落盘） | 无权限 |
...
- 控制端口不运行业务操作；`POST /api/bug` 只记录原始请求体到本地工作目录 `log/bug_reports.jsonl`，不校验格式、不解析内容、不触发业务，成功返回 2xx 收据。
