...
3. **结果要"能读"**：`read_results` 直接给分类计数、前 N 条违规（规则名/坐标/层）、LVS match/差异点、PEX warning 清单，而不是让用户去啃 100 KB 报告；
...
|---|---|
| `kind` | `drc` / `lvs` / `pex` |
| `summary` | DRC：`{total_results, rules_checked, by_rule:{规则名:条数}}`；LVS：`{status: match/incorrect/unknown, device_counts, net_counts, differences:[…]}`；PEX：`{errors, warnings, netlist_files, pdb_dirs}` |
| `first_offenders` | DRC：前 `limit` 条 `{rule, layer, cell, bbox, count}`；LVS：差异点条目 |
