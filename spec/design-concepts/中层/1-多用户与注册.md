...
> Supersedes：Draft v14（改名多用户与注册，聚焦注册：六步/授权/参数；路由侧重移交路由设计）  
> 关联：[四层整体架构与接口](../总览/1-四层整体架构与接口.md)、[路由设计](3-路由设计.md)、[Spec 索引](../../README.md)

## 1. 目标

把一个**单连接点对点桥接**演进为**多用户接入架构**：

- 每个用户拥有独立的 resident daemon、独立监听端口；连接复用与预算的唯一口径见[路由设计 §4](3-路由设计.md)与[并发设计](2-并发设计.md)；
...
> **N 个用户 = N 个隔离域**。例如 6 个用户，上层看到的是 6 个业务服务器；每个隔离域包含五个 role、一个活动 daemon（= 一个 CIW）和一份 per-token 线程/channel 预算，中层按 token 参数路由到对应隔离域。跨用户并行互不阻塞；同账号场景下是桥层通道隔离（命令流不串、返回不混、文件落在各自 role 的根下）；**token 是核心区分**，OS 级权限分离由 IT 提供 Unix 账号保证，不在桥的职责内。接口清单与 role 映射见[四层整体架构与接口 §4.1](../总览/1-四层整体架构与接口.md)与[路由设计 §3](3-路由设计.md)，本文不重复。
...
本设计把上述机制**统一收拢**为六步注册流程（详见 3.2）：**①申请 → ②本地校验 → ③探测 → ④部署 → ⑤连通性测试 → ⑥写入注册表**。注册完成后按注册表路由；SSH 连接在 token 内按 endpoint 去重复用（唯一口径见[路由设计 §4](3-路由设计.md)与[并发设计](2-并发设计.md)）。
...
注册是一条**先验证、后落盘**的流水线：前五步任何一步失败，注册表都不产生条目；第六步是唯一写入点。参数细节见 [中层配置文档 §4](../总览/add-中层配置文档.md)，此处聚焦流程本身。
...
| 步 | 预测目标（可观察） | 不达成时 | 重试 |
|---|---|---|---|
| 1 申请 | 必填参数齐全（必填项以[中层配置文档 §4](../总览/add-中层配置文档.md)为准：至少 `mode.default`、`user`；每个 remote role 必须可解析出目标） | 参数错误，无副作用 | 修改后重新申请 |
| 2 本地校验 | registry 内无同名 user；`local_port` 在本机未被占用；`daemon_port` 若已提供，在**同一 daemon 目标主机**上查重（缺省端口待第三步分配） | 返回冲突原因 | 改参数后重试 |
| 3 探测 | 逐 role 探测矩阵与部署根的唯一 owner：本文 §12；`expected_*` 固化、`role.*.root` 回写（spectre 失败时相关字段为 null） | ERROR（spectre role 整体仅 WARNING） | 修复环境后重试 |
...
用户发起注册请求，提交**必选参数**（以[中层配置文档 §4](../总览/add-中层配置文档.md)为准；至少 `mode.default`、`user`，每个 remote role 必须可解析目标）；`mode.default ∈ {local,remote}` 是强制字段，不自动推断；`role.*.mode` 可逐 role 覆盖。本步只是"提出申请"，不产生任何副作用。token 在此时生成（自动或手动指定），随申请在流程内部传递，第六步才落盘。

> **参数清单**：完整参数表见 [中层配置文档 §4](../总览/add-中层配置文档.md)——本文件只关注流程，参数细节不在此展开。
...
任一**必检**项失败 → **回退第一步**，返回结构化原因（如 `ssh handshake failed`、`daemon port 65081 already in use`、`role.daemon.root not writable`）。spectre role 的探测失败只产生 WARNING，不阻断注册。逐 role 的职责、探测矩阵与文件根唯一 owner：[路由设计](3-路由设计.md)。
...
**同 endpoint 可能被多个 `mode=remote` 的 role 共享**：每个 role 各自持有 `role.<name>.expected_fingerprint`，**同 endpoint 的多份值必须一致**（注册时校验；key 算法见[中层配置文档 §6.5](../总览/add-中层配置文档.md)）。gui/daemon/command/file 的指纹不匹配 → ERROR；spectre 走 Spectre 命令执行接口（上层预留），其指纹缺失/不匹配只 WARNING。
...
- **字段组成**：token、`mode.default`/`role.*.mode`、五个 role（gui/daemon/command/file/spectre，连接字段缺省回退全局 `ssh.default.*`，各 role 自带 `expected_*` 探测基准）——**字段与示例见 [中层配置文档 §2](../总览/add-中层配置文档.md)**，此处不重复；
...
五个接口各自投送到对应 role：Skill→daemon（daemon 校验 token，NAK 时 SKILL 零接触）、命令→command、文件→file、GUI→gui、Spectre→spectre；后四类只做**寻址**，不额外做 token 安全校验（安全边界 = SSH host key + 账号权限）。唯一映射与 token 语义见[路由设计 §3](3-路由设计.md)与[四层整体架构与接口 §4](../总览/1-四层整体架构与接口.md)，本文不复制表格。
...
| 手动 token 约束 | **已定**：`^[A-Za-z0-9._-]{1,64}$`（见[中层配置文档 §5](../总览/add-中层配置文档.md)） |
...
| 一个 token 多主机 | **支持 role 分布多主机**（五 role 可跨机、可混合 local/remote）；只禁止同一 token 出现**第二个 daemon / 第二个 CIW**。唯一口径：[路由设计 §1.1](3-路由设计.md) |
...
## 12. 注册阶段的探测矩阵与部署根（唯一 owner）

> 本内容原属《路由设计》，按“路由是纯消费注册表的行为”迁入：探测、部署、根写回都发生在**注册阶段**；路由阶段只消费注册表里已写回的字段。
...
**每个 role 都有自己的文件根（root），该 role 的所有文件需求都在自己的根下处理。** 根的计算**与 role 的 mode 无关**，只与“该 role 实际在哪台机器上工作”有关：

1. **基准**：`root.default` 未显式提供时等于 `~/.virtuoso-bridge/<userid>`；`~` 与 `<userid>` 都在该 role 实际工作所在机器上解释：`mode=remote` 时 `~` 是该 role SSH 登录账号的 HOME；`mode=local` 时是运行中层的账号的 HOME（不是客户端工作目录、不是 registry 目录）。
2. **role 根**：`role.<name>.root` 未显式配置时 = `<root.default>/<role>`；显式配置时该值就是最终根，不再追加用户名或 role 名。
3. **`~` 展开**：只在访问文件系统时展开，展开主体必须是该 role 实际使用的账号；解析不了属于路径错误（`VB-PATH-NOT-VISIBLE`），不得退化成字面 `~` 目录。
4. **注册表存什么**：未显式提供时 `root.default` 存空值；注册探测完成后把展开后的绝对路径写回各 `role.root`，运行期不再重新解析 `~`、也不依赖远端环境变量。
5. **运行期路径解析**：各 role 的初始 cwd 与 `remote_path` 相对路径共用同一解析——绝对路径原样使用；相对路径 = 该 role 的最终根 + 相对路径；`~` 开头按第 3 条展开。

- bridge 文件（daemon / il / setup）**只部署到 daemon role 的根**（`role.daemon.root`），上传动作走 daemon role 的连接；bridge 不检查该根是否对 gui 可见（CIW 能否 `load` 由用户拓扑负责）；
- 相对路径按目标 role 的根解析；绝对路径不受限制；常驻 shell 建立时以该 role 根为初始 cwd；各 role 根互相独立、无可见性要求。
