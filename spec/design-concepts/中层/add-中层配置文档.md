...
- **运行环境要求**：客户端（顶层/上层/中层运行侧）Python ≥ 3.9；五个 role 目标机器 Python 2.7 及以上或 3.6.8 及以上（remote=SSH 账号环境，local=运行中层的本机）；`role.daemon.python` 的显式校验与缺省探测按此版本口径。
...
{"alice": {
  ...
  "roles": {
    ...
    "command": {"root": "/home/ssh-user/.virtuoso-bridge/alice/command", "max_sessions": 10,
                "calibre": {"bin": "/opt/eda/mentor/calibre/bin/calibre", "version": "2024.4"},
                "expected_fingerprint": "SHA256:…"},
...
**流程**：第二步在内存分配候选（缺省 `local_port` 本机预分配；缺省远端 `daemon_port` 第三步分配并回写）→ 第四步部署前复核端口占用、冲突重分配一次 → 第六步提交前 final re-check → **失败不释放**；`cancel` 或服务重启释放；第六步提交成功释放候选。
...
- 同 key 同 endpoint 且**解析后凭据一致**：共享业务连接与 fingerprint 基准（拓扑见[路由设计 §4](3-路由设计.md)）；不同 key 不共享；key 变更视为新 endpoint，指纹按首信任重建；同 key 多 role 显式 `expected_fingerprint` 不一致 → 注册 ERROR；
