...
> Supersedes：v17（`role.gui.display` 消费与单 X server 边界）
...
- GUI 命令用于 X11/窗口/CIW 相关的一次性操作；中层执行时按该 token 的 `role.gui.display` 设置 `DISPLAY`，字段为 `null` → 执行失败并在错误中指明 GUI display 未配置；
