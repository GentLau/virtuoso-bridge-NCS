        ...
        *,
        ...
        if runner is not None:
            ...
            if entry.topic or entry.func_name:
                # Extract the topic: modern TOPIC markers first, then legacy
                # anchor/heading location.  A NULL topic means the whole file
                # is the docs, but try a precise section first (cuts page
                # header/nav noise on legacy pages) and only degrade to the
                # whole file when the section cannot be located.
                from virtuoso_bridge.virtuoso.skill_finder.more_info import (
                    extract_doc_section,
                )
                single_func_file = sum(
                    1 for e in entries.values() if e.file_path == entry.file_path
                ) == 1
                ...
                # No topic and no function name — return the whole file
