...
def post_retry(base, path, payload, retries=60, timeout=120):
    reject_retries = 0
    http_retries = 0
...
def main(argv=None):
    ...
    def job(kind, i):
        ...
        if kind == "command":
            if i % 3 == 0:
                cmd = f'"{sys.executable}" -c "print(sum(range(1,101)))"'
                want = "5050"
            elif i % 3 == 1:
                cmd = f"echo vb-{i}"
                want = f"vb-{i}"
            ...
            if status == 200 and str(body.get("stdout", "")).strip() != want:
                cmd_stats.bad_value += 1
        elif kind == "skill":
            lat, status, body, reject_retries, http_retries = post_retry(
                args.base, "/api/skill", {"token": tok, "skill": "1+1"},
            )
            skill_stats.add(lat, status, body, reject_retries, http_retries)
            out = str(body.get("output", "")).strip().strip('"')
            if status == 200 and out != tok:
                # output must come from the daemon that owns this token
